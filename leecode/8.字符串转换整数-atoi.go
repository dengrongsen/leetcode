...
func myAtoi(s string) int {
	...
	for _, v := range []byte(s) {
		...
		v -= '0'
		res = res*10 + int64(v)
		if s0[0] != '-' && res >= math.MaxInt32 {
			res = math.MaxInt32
			invalid = true
			break
		}
		if s0[0] == '-' && -res <= math.MinInt32 {
			res = math.MinInt32
			invalid = true
			break
		}
...
/* 
总结：
最初版本将字符串转为整数用了`strvconv.Atoi(string) int`，
后面改为自己实现Atoi，原理也不难，采用跟第7题的思路，只不过这里是从第一位取，
逐个拼接(`res*10 + int64(v)`)成整数。
字符串s，可以采用`for _, v := range []byte(s)`去遍历，而如果字符串s包含unicode字符集的
字符，必须使用`for _, v := range []rune(s)`去遍历。
*/